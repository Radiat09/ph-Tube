...
const cardElement = (element) => {
  ...
  card.classList = `2xl:w-[314px] md:w-[300px] `;
  let verifiedIcon = "";
  if (element?.authors[0]?.verified) {
    verifiedIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
const handleCatagory = async () => {
  ...
  catagoryData.forEach((element) => {
    ...
    tabDiv.innerHTML = `
    <a id="${element?.category_id}" onclick="loadSpecificCatagory('${element?.category_id}'),loadingSpinner('true')" class="tab">${element?.category}</a>
    `;
    tabContainer.appendChild(tabDiv);
  });
};

const sortByViewBtn = (getCatagoryId = 1000) => {
  const sortByViewDiv = document.getElementById("sort-by-view");
  const aTag = document.createElement("a");
  aTag.classList = `btn hover:btn-active text-sm md:text-xl`;
  aTag.innerHTML = `
      <button onclick="sortByView('${getCatagoryId}')">Sort by view</button>
    `;
  sortByViewDiv.appendChild(aTag);
};
sortByViewBtn();
// Load all catagory
...
// Sort By View
const sortByView = async (catagoryId) => {
  loadingSpinner(true);
  const CardContainer = document.getElementById("card-container");
  ...
  let newSort = allCatagory.sort((a, b) => {
  ...
  card.innerHTML = `
    ...
    ${element?.title}
      ...
      <div class="flex justify-center items-center gap-2">
        ...
        <p class="text-second">${element?.others?.views}K Views</p>
    </div>
    </div>
    `;
